...
const isDisabled = computed(() => {
  ...
  if (props.field.dependsOn?.length) {
    return props.field.dependsOn.some((dep: string) => !props.formValues?.[dep])
  }
...
function buildApiUrl(): string {
  ...
  if (props.field.props?.queryParams && props.formValues) {
    try {
      ...
      console.error(`Error building query params for ${props.field.name}:`, err)
    }
  }
...
async function fetchOptions(skipCache = false) {
  ...
  try {
    ...
    const items = Array.isArray(data)
      ? data
      : data.data || data.provinces || data.districts || data.communes || data.villages || []
...
function handleChange(selectedOption: any) {
  ...
  const rawObject = selectedItem?._raw || { code: selectedValue, name_kh: selectedItem?.label }
...
function handleChangeEvent(event: Event) {
  ...
  const rawObject = selectedItem?._raw || { code: currentValue, name_kh: selectedItem?.label }
