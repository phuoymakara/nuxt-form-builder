    ...
    <div v-for="rowNumber in uniqueRows" :key="rowNumber" class="grid gap-6" :class="getGridClass(rowNumber)">
      ...
      >
        ...
        <BaseAddress
          ...
          :ref="(el) => { if (el) addressFieldRefs[field.name] = el }"
          :id="field.name"
          :field="field"
          :model-value="values[field.name]"
        ...
        <UFileUpload
          v-else-if="field.component === 'UFileUpload' && field.type === 'file' && field.name === 'avatar'"
          :field="field"
          accept="image/*"
...
const onFieldChange = (value: any, field: FieldWithConditions, index: number) => {
  ...
  if (field.component === 'UAddress' && typeof value === 'object' && value !== null && value.code) {
    finalValue = value
  }
  ...
  const { valid, message } = validateField(field.name, finalValue, field.validation)
  errors.value[field.name] = valid ? undefined : message
...
const clearDependentFields = (changedFieldName: string) => {
  props.fields.forEach((field: FieldWithConditions) => {
    if (field.dependsOn?.includes(changedFieldName)) {
      ...
      if (shouldClear) {
        values.value[field.name] = field.component === "UCheckboxGroup" ? [] : ""
